...
class HyperPINN(nn.Module):
    def __init__(self, activation_function, hard_constraint_fn=None, p_dropout=0.2, encoding=None) -> None:
        super().__init__()
        
        self.hard_constraint = hard_constraint_fn
        
        self.main = MLP([1] + [204]*5 + [2], activation_function,None, p_dropout=0.0, encoding=None)
        ...
        for name ,param in self.main.named_parameters():
            if "weight" in name:
                self.num_weights += param.numel()
            else:
                self.num_bias += param.numel()
                
        
        self.num_parameters = self.num_bias+self.num_weights
      
        self.hyper = MLP([2] + [308]*8 + [self.num_parameters] ,activation_function,None,p_dropout=0.0,encoding=None)
    
    def forward(self, x):
        orig_x = x
        
        weights = self.hyper(orig_x[0:2])
        ...
        with torch.no_grad():
            layers_list = self.main.layers
            for i in range(len(layers_list) - 1):
                
                cur_idx = next_idx
                next_idx += layers_list[i] * layers_list[i + 1]
    
                weights_splice = weights[cur_idx:next_idx].reshape([layers_list[i + 1], layers_list[i]])

                # dim_bias = self.main.mlp[i*3].bias.size(dim=0)
                dim_bias = layers_list[i+1]
             
                bias_splice = weights[self.num_weights + shift_bias: self.num_weights + dim_bias + shift_bias].reshape([dim_bias])
                shift_bias += dim_bias
                
                
                # del self.main.mlp[i * 3].weight
                self.main.mlp[i * 3].weight = weights_splice
                # del self.main.mlp[i*3].bias
                self.main.mlp[i*3].bias = bias_splice

        
        output = self.main(orig_x[2:3])
        if self.hard_constraint != None:
             output = self.hard_constraint(orig_x, output)
